            ...
            if ($user->hasRole(['Admin', 'Super Admin'])) {
                $transaction_list = DB::table('transaction_log')->where(['transaction_log.is_active' => 1])
                    ...
                    ->select(
                        ...
                        DB::raw('ifnull((transaction_log.charged_battery_bv_1 * transaction_log.charged_battery_rec_1) / 1000,0) - ifnull((transaction_log.discharged_battery_bv_1 * transaction_log.discharged_battery_rec_1) / 1000,0) as diffcharge1'),
                        DB::raw('ifnull((transaction_log.charged_battery_bv_2 * transaction_log.charged_battery_rec_2) / 1000,0) - ifnull((transaction_log.discharged_battery_bv_2 * transaction_log.discharged_battery_rec_2) / 1000,0) as diffcharge2'),
                        'transaction_log.*',
