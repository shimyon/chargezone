...
class SwappingTransactionExport implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        $transaction_list = DB::table('transaction_log')->where(['transaction_log.is_active' => 1])
            ...
            ->addSelect(
                'discharged_battery_bin_1',
                'discharged_battery_soc_1',
                'discharged_battery_bv_1',
                'discharged_battery_rec_1',
                // DB::raw('(discharged_battery_bv_1*discharged_battery_rec_1) AS kwh1')
                DB::raw('ifnull((transaction_log.discharged_battery_bv_1 * transaction_log.discharged_battery_rec_1) / 1000,0) as discharge1_kwh')
            )
            ->addSelect(
                'discharged_battery_bin_2',
                'discharged_battery_soc_2',
                'discharged_battery_bv_2',
                'discharged_battery_rec_2',
                // DB::raw('(discharged_battery_bv_2*discharged_battery_rec_2) AS kwh2')
                DB::raw('ifnull((transaction_log.discharged_battery_bv_2 * transaction_log.discharged_battery_rec_2) / 1000,0) as discharge2_kwh')
            )
            ->addSelect(
                'end_time',
                'charged_battery_bin_1',
                'charged_battery_soc_1',
                'charged_battery_bv_1',
                'charged_battery_rec_1'
                // , DB::raw('(charged_battery_bv_1*charged_battery_rec_1) AS kwh3')
                ,
                DB::raw('ifnull((transaction_log.charged_battery_bv_1 * transaction_log.charged_battery_rec_1) / 1000,0) as charge1_kwh')
            )
            ->addSelect(
                'charged_battery_bin_2',
                'charged_battery_soc_2',
                'charged_battery_bv_2',
                'charged_battery_rec_2',
                // DB::raw('(charged_battery_bv_2*charged_battery_rec_2) AS kwh4')
            ...
            ->addSelect(
                ...
                DB::raw('( ( ifnull( (charged_battery_bv_1 * charged_battery_rec_1) / 1000,0) - ifnull( (discharged_battery_bv_1 * discharged_battery_rec_1) / 1000,0) ) + ( ifnull( (charged_battery_bv_2*charged_battery_rec_2)  / 1000,0) - ifnull( (discharged_battery_bv_2*discharged_battery_rec_2)  / 1000,0) )) AS energy')
            )
